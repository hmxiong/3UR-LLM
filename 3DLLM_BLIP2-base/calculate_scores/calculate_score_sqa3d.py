...
if __name__=="__main__":
    ...
    golds = json.load(open(os.path.join('/13390024681/3D/3D-LLM/data/SQA3D/ScanQA_format/SQA_test.json')))
    ...
    pc_feat_root = "/13390024681/3D/3D-LLM/data/scannet_data"

    # golds = [ann for ann in golds if "scene_id" in ann and os.path.exists(os.path.join(pc_feat_root, ann["scene_id"] + ".pt"))]
    golds = [ann for ann in golds if "scene_id" in ann and os.path.exists(os.path.join(pc_feat_root, ann["scene_id"] + "_vert.npy"))]

    preds_={k:{} for k in QT}
    golds_={k:[] for k in QT}

    assert len(golds) == len(list(preds.keys())), print(len(golds), len(list(preds.keys())))
