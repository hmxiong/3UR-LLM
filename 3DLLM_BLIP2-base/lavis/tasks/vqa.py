...
class VQATask(BaseTask):
    ...
    def valid_step(self, model, samples):
        answers, boxes_unnormalized, feature_vis_save = model.predict_answers(
            samples=samples,
        ...
        if 'scene_id' in samples.keys():
            # print(samples['scene_id'])
            scene_id = samples['scene_id'][0]
            
        ids = samples["id"] if "id" in samples.keys() else [x for x in range(len(question_id))]
        questions = samples["text_input"]
        right_answers = samples["answers"] if "answers" in samples.keys() else "There is no right answer"
        for answer, box, question, ques_id, id in zip(answers, boxes_unnormalized, questions, question_id, ids):
            ...
            # print(answer)
            # print(right_answers)
            # if os.path.exists(os.path.join('/13390024681/3D/3D-LLM/3DLLM_BLIP2-base/lavis/models/feature_vis', scene_id+'.npy')):
            #     print("{}exist".format(os.path.join('/13390024681/3D/3D-LLM/3DLLM_BLIP2-base/lavis/models/feature_vis', scene_id+'.npy')))
            # else:
            #     np.save(os.path.join('/13390024681/3D/3D-LLM/3DLLM_BLIP2-base/lavis/models/feature_vis', scene_id+'.npy'), features)
            
            if scene_id is None:
                pred_qa_pairs.append({"id":id, "boxes":box, "question_id":ques_id, "question":question, "answer": answer, "right_answers": right_answers})
            else:
                pred_qa_pairs.append({"id":id, "boxes":box, "scene_id":scene_id, "question_id":ques_id, "question":question, "answer": answer, "right_answers": right_answers})
