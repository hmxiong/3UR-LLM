...
@registry.register_model("blip2_3detr_rl")
class Blip2ThreeDETR_RL(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        if self.status == 'finetune':
            if self.use_cot:
                ...
                with open("/13390024681/3D/3D-LLM/data/all_scene_caption.json") as f:
                    self.cot_prompt = json.load(f)
                self.cot_instruction = 'Based on the description below:{description}. Please provide answer to the questions:'
                self.judge_instruction = "The following is the description provided:'{description}'. Please judge based on the information provided that you can accurately answer the following questions: '{question}' You only need to answer 'Yes' or 'No'"
                # 整体的训练速度很慢啊感觉
        ...
        self.vicuna_model, self.vicuna_tokenizer = self.init_vicuna(vicuna_model)
    ...
    def init_vicuna(self, vicuna_path):
        ...
        return vicuna_model, vicuna_tokenizer

    def vicuna_judgement_training(self):
    ...
    def from_config(cls, cfg):
        ...
        vicuna_path = cfg.get("vicuna_path", "")
