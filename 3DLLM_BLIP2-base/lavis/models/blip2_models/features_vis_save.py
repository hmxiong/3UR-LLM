...
class feature_vis():
        ...
        for i in range(B):
            for j in range(N):
                rgb_score[i][j][:] = self.color_mapping(
                    score=feature_score[i][j][0], color_type='OrRd') # YlGnBu OrRd
        return rgb_score
    ...
    def feature_save(self, xyz, features, layer_name='layer_1'):
        ...
        '''
        if xyz is not None:
            xyz = np.array(xyz.detach().cpu())
            B, N, Coord = xyz.shape
        features = np.array(features.detach().cpu())
        _, C, _ = features.shape
        features = np.transpose(features, (0, 2, 1)) # B N C
        ...
        feature_vis_save = np.concatenate((xyz, features_rgb), axis=2) if xyz is not None else features_rgb
        filename_base = 'feature_' + layer_name
        
        # np.save(os.path.join(self.save_path, filename_base), feature_vis_save)
        return feature_vis_save
