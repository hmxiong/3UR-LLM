...
@registry.register_model("blip2_3detr_concate_more")
class Blip2ThreeDETR_Concate_more(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.inject_proj = nn.Linear(self.visual_encoder.hidden_size, self.t5_model.config.hidden_size)
    ...
    def forward(self, samples):
        ...
        inject_feature = self.inject_proj(chosen_pcl_query)
        ...
        # print("inject_feature", inject_feature.shape) # bs 256 2048
        # print("enc_features", enc_features.shape)
        # print("all_pcl_feature", all_pcl_feature.shape)
        # print("chosen_pcl_query", chosen_pcl_query.shape)
        ...
        inputs_t5 = torch.cat([inputs_t5, inject_feature], dim=1).to(all_pcl_feature.device) # 32+256
        atts_t5 = torch.ones(inputs_t5.size()[:-1], dtype=torch.long).to(all_pcl_feature.device)

        # print("inputs_t5", inputs_t5.shape)
        # print("n_answers", len(samples["n_answers"]))
        # print("samples", samples["n_answers"])
        # print("answers", samples["answer"])
    ...
    def predict_answers(
        ...
        **kwargs,
        ...
        inject_feature = self.inject_proj(chosen_pcl_query)
        chosen_pcl_query = self.pcl_query_proj(chosen_pcl_query)
        # inject_feature = self.inject_proj(pcl_feature)
        pcl_feature = self.vision_proj(pcl_feature) # project pcl dimension to llm dimension
        enc_features = self.enc_vision_proj(enc_features) # bs 4096 1048 # maypbe this has too many datasets
        all_pcl_feature = torch.cat([pcl_feature, enc_features], dim=1) # bs 4352 1408
        # all_pcl_feature = enc_features
        ...
        inputs_t5 = self.t5_proj(query_output.last_hidden_state)
        inputs_t5 = torch.cat([inputs_t5, inject_feature], dim=1).to(all_pcl_feature.device)
