...
@registry.register_model("blip2_vdetr_opt")
class Blip2VDETROPT(Blip2Base):
    """
    BLIP2 OPT model.
    ...
    def __init__(
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(num_query_token, 1408) # this must be 1408 dim 
    ...
    def forward(self, samples):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            inputs_embeds = self.opt_model.encoder.embed_tokens(batch_input_tokens_input_ids)
    ...
    def generate(
        ...
        input_tokens = self.opt_tokenizer(prompt, padding="longest", return_tensors="pt").to(all_pcl_feature.device)

        encoder_atts = torch.cat([atts_opt, input_tokens.attention_mask], dim=1)

        device_type = "cuda" if "cuda" in str(self.device) else "cpu"
        with torch.cuda.amp.autocast(enabled=(self.device != torch.device("cpu")), dtype=torch.float32):
            inputs_embeds = self.opt_model.encoder.embed_tokens(input_tokens.input_ids)
    ...
    def predict_answers(
        ...
        **kwargs,
        ...
        input_tokens = self.opt_tokenizer(text_input, padding="longest", return_tensors="pt").to(all_pcl_feature.device)
