...
from lavis.models.blip2_models.features_vis_save import feature_vis
...
class Blip2ThreeDETR_Concate(Blip2Base):
    ...
    def __init__(
        ...
        self.freeze_q_former = freeze_q_former
        ...
        if self.freeze_q_former:
            logging.info("Freeze Q-Former during training !!")
    ...
    def predict_answers(
        ...
        **kwargs,
        ...
        pcl_feature_int=F.interpolate(enc_features.permute(0, 2, 1) , size=[4000], mode='nearest')
        # print('pcl_feature_int',pcl_feature_int.shape)
        
        # feature_vis_save = self.Visualize_feature.feature_save(pc_embeds.to(torch.float32)[:,:,:3], pcl_feature_int)
        feature_vis_save = None
        # assert 1==2
        # print(box_predictions['outputs'].keys())
        
        values, index = box_predictions["outputs"]["objectness_prob"].topk(self.num_query_token, dim=1, largest=True, sorted=True)
        
        size_unnormalized   = box_predictions['outputs']['size_unnormalized'].index_select(dim=1, index=index[0])
        center_unnormalized = box_predictions['outputs']['center_unnormalized'].index_select(dim=1, index=index[0])
        # print('size_unnormalized',  size_unnormalized.shape)   # bs num_query 3
        # print('center_unnormalized',center_unnormalized.shape) # bs num_query 3
        
        box_unnormalized = torch.cat([center_unnormalized, size_unnormalized], dim=-1) # bs num_query 6
        box_unnormalized = box_unnormalized.detach().cpu().numpy().tolist()
        # print(box_unnormalized[0])
        
        # print('box_unnormalized', box_unnormalized.shape)
        ...
        return output_text,box_unnormalized,feature_vis_save
