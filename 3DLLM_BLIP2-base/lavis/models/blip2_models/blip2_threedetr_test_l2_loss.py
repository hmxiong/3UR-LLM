...
@registry.register_model("blip2_3detr_l2_loss")
class Blip2ThreeDETR_L2Loss(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        logging.info(f'shared qformer -- 使用l2 norm正则化约束')
        self.modality_qformer_12 = copy.deepcopy(self.Qformer)
        self.modality_query_tokens = copy.deepcopy(self.query_tokens)
    ...
    def l2_loss(self, model, model_ref):
        Loss = 0.0
        for param_q, param_k in zip(model.named_parameters(), model_ref.named_parameters()):
            #查看一下参数名，将frozen的给去掉
            # print(param_q[0],param_q[1].grad,param_k[1].grad)
            Loss += F.mse_loss(param_q[1].sum(), param_k[1].sum().detach(), reduction="sum")
        return Loss
    ...
    def forward(self, samples):
        ...
        l2_loss = self.l2_loss(self.Qformer, self.modality_qformer_12)
        l2_loss_query = self.l2_loss(self.query_tokens, self.modality_query_tokens)
        logging.info('l2_loss :{}'.format(l2_loss, l2_loss_query))
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            loss = outputs.loss
            return {"loss": loss+l2_loss+l2_loss_query}
