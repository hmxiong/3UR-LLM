...
class RunnerBase:
    ...
    def __init__(self, cfg, task, model, datasets, job_id):
        ...
        self.start_epoch = 0
        self.eval_epochs = int(self.config.run_cfg.eval_epochs)
        self.save_epochs = int(self.config.run_cfg.save_epochs)
    ...
    def train(self):
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            # evaluation phase
            if len(self.valid_splits) > 0 and (cur_epoch+1) % self.eval_epochs == 0:
                if (cur_epoch+1) % self.save_epochs == 0:
                    self._save_checkpoint(cur_epoch, is_best=False) # only save after eval
                for split_name in self.valid_splits:
                    logging.info("Evaluating on {}.".format(split_name))
