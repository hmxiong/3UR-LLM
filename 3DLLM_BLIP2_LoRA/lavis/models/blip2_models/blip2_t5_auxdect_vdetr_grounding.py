...
@registry.register_model("blip2_t5_auxdect_vdetr_grounding")
class Blip2T5_auxdect_vdetr_grounding(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        # print("len(self.t5_tokenizer)", len(self.t5_tokenizer))
        location_token = '[location]'
        # self.t5_tokenizer.add_special_tokens({"additional_special_tokens": location_token})
        self.t5_tokenizer.add_tokens(location_token)
        
        self.box_loc_id = self.t5_tokenizer(location_token, add_special_tokens=False).input_ids[0]
        print("self.box_loc_id", self.box_loc_id)
        # print("len(self.t5_tokenizer)", len(self.t5_tokenizer))
        # assert 1==2
    ...
    def forward(self, samples):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            location_token_mask = output_tokens.input_ids == self.box_loc_id
            target_mask = targets != -100
            final_mask = (target_mask & location_token_mask) # bs num_seq
            ...
            # box regression loss
            decoder_hidden_states = outputs.decoder_hidden_states[-2]
            bs, nseq, ndim = decoder_hidden_states.shape
            select_hidden_sttate =  decoder_hidden_states[final_mask].unsqueeze(1) # bs ndim
            box_seq_center = self.center_head(select_hidden_sttate.transpose(1, 2)).squeeze(2) # bs 3 
            box_seq_size_reg = self.size_head(select_hidden_sttate.transpose(1, 2)).squeeze(2) # bs 3 
            box_seq_size = torch.exp(box_seq_size_reg)
            
            target_box = samples['box']
            target_box = torch.stack(target_box, dim=0).to(box_seq_center.device) # bs 6
            target_boxes_center = target_box[:,:3] # bs 3
            target_boxes_size = target_box[:,3:] # bs 3
            center_loss = nn.functional.l1_loss(box_seq_center,target_boxes_center) 
            size_loss = nn.functional.l1_loss(box_seq_size_reg,torch.log(target_boxes_size))
            # print("center_size_loss", center_loss, size_loss)
            # assert 1==2
            # return {"loss": loss
            
            
            box_losses = center_loss + size_loss
