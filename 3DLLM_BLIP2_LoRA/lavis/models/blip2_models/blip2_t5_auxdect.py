...
@registry.register_model("blip2_t5_auxdect")
class Blip2T5_auxdect(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def forward(self, samples):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for j in range(pc.shape[0]):
                ...
                for i in range(3):
                    pc_i = pc[j][:, i]
                    pcs.append(self.pos_embedding[pc_i])#embeding: [256,469] 
                pcs = torch.cat(pcs, -1)
                all_pcs[j][:, :1407] = pcs
        ...
        query_tokens = self.query_tokens.expand(pc_embeds.shape[0], -1, -1)  # bs,32,768
        ...
        inputs_t5 = self.t5_proj(query_output.last_hidden_state) #768->2048
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for b, n in enumerate(samples["n_answers"]):
                ...
                batch_box_info += [samples['detect_info'][b]] * n
