...
class Blip2T5(Blip2Base):
    ...
    def forward(self, samples):
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            # print("pc_embeds:", pc_embeds.dtype) # 2 5000 1408
        ...
        if self.query_text_output:
            ...
            query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(pc_embeds.device)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for b, n in enumerate(samples["n_answers"]): # number of answers
