...
from lavis.models.backbone_module import Pointnet2Backbone
from lavis.models.blip2_models.modeling_t5 import T5Config, T5ForConditionalGeneration
from positional_encodings.torch_encodings import PositionalEncoding1D


@registry.register_model("blip2_pcl")
class Blip2PCL(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.visual_encoder = self.init_pcl_encoder(pretrain=True, store_path="/13390024681/3D/3D-SPS/xyz_rgb_norm_backbone.pth")
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(num_query_token, 288)
    ...
    def init_pcl_encoder(self, pretrain=True, store_path="../model_zoo/VDETR/", num_token=256, device="cpu"):
        if pretrain and not os.path.exists(store_path):
            raise ValueError(f"VGNet pretrained model not found at [{store_path}]!")  # 目前需要再进一步等待checkpoint给我发过来
        ckpt = torch.load(store_path, map_location=device)
        
        print("===> Loading Pointnet-2 encoder...\n")

        model = Pointnet2Backbone(input_feature_dim=6,
                                  output_feature_dim=288)
    ...
    def forward(self, samples):
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            # print("pc_embeds:", pc_embeds.shape) # 2 5000 1408
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for j in range(pc.shape[0]):
                ...
                pcs = torch.cat(pcs, -1)
                all_pcs[j][:, :1407] = pcs
            all_pcs = all_pcs.cuda() # 需要做的是添加进position embedding

        pc_embeds = pc_embeds + 0.01 * all_pcs
        print("pc_embeding", pc_embeds.size()) # bs nmum_squence channel
